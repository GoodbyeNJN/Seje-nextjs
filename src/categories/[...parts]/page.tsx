...
import { joinInAbsolute, splitWithoutBase } from "@/utils/url";
...
const getCategory = async (params: Param) => {
    const { parts } = params;
    const pathname = joinInAbsolute(FIRST_PART, ...parts);
...
export const generateStaticParams = async (): Promise<StaticParams> => {
    const params = (await getCategories()).map(({ slug }) => ({
        parts: splitWithoutBase(slug).filter(part => part !== FIRST_PART),
    }));
