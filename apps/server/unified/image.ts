...
import { isString } from "share/utils";
...
export const rehypeImage =
    (options?: ImgSizeOptions): Transformer<Root> =>
    async (tree, file, next) => {
        ...
        await Promise.all(elements.map(node => visitor(node, options?.dir)));
